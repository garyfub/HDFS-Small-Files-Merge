...
class InitConfig() {
  ...
  def initDimFrontCate(sqlContext: HiveContext): mutable.HashMap[String, String] =
  {
    ...
    val sql = s"""select front_cate_id, case when (pid = 490 or front_cate_id = 490) then level_id +1 when (front_cate_id=488 or pid =488) then level_id+2 else level_id end level_id
