...
-- -------------------------------------------------------------------------------------------------
-- 插入APP端PAGE数据
-- 154 page_active  select getpageid('http://m.juanpi.com/act/sub_shumabk618?mobile=1&qmshareview=1&qminkview=1');    结果10069
-- 289 page_h5      select getpageid('http://m.juanpi.com/help/shopAdvice?pid=214');  jieguo -1
-- 289 page_h5      select getpageid('http://m.juanpi.com/coudan?mobile=1&actid=21312');  jieguo -1
-- 289 page_h5      select getpageid('http://m.juanpi.com/help/customer');  jieguo  10023

-- kaikai  当pagename为空且extends_params为空时，在app端则属于卷皮站内未知页。
-- APP端的H5页面则是wap_url字段存放，APP内不会有站外页。 wap_url=about:blank是个特殊的页面
-- -------------------------------------------------------------------------------------------------
